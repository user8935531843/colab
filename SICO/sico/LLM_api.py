...
def get_llm_api(llm_name_str, input_device):
    if llm_name_str == 'chatgpt':
        ...
        from transformers import AutoTokenizer, AutoModelForCausalLM
        model_path = "/content/drive/MyDrive/SICO/models/vicuna-7b-v1.5"
        print(f"Loading Vicuna model from: {model_path}")
        try:
            tokenizer = AutoTokenizer.from_pretrained(
                model_path,
                use_fast=False,
                local_files_only=True # Dodano argument
            )
            ...
            llm_api = HFLLMAPI(tokenizer, model, input_device=input_device, max_length=2048)
        except Exception as e:
             print(f"BŁĄD podczas ładowania modelu Vicuna z {model_path}: {e}")
             print("Upewnij się, że model został poprawnie pobrany do tej lokalizacji i zawiera wszystkie potrzebne pliki (config.json, pytorch_model.bin, tokenizer_config.json itp.).")
             raise e
    else:
...
class HFLLMAPI(LLMAPI):
    ...
    def __call__(self, input_text, max_new_tokens, return_num, generation_kwargs):
        ...
        if max_new_tokens is None or not isinstance(max_new_tokens, int) or max_new_tokens <= 0:
             print(f"Ostrzeżenie: Otrzymano nieprawidłową wartość max_new_tokens ({max_new_tokens}). Ustawiam domyślną = 512.")
             max_new_tokens_calc = 512 # Bezpieczna wartość domyślna
        else:
            max_new_tokens_calc = max(1, min(max_new_tokens, self.max_length - input_len)) # Oblicz i zapewnij minimum 1

        # Sprawdzenie, czy jest w ogóle sens generować (czy input nie zajmuje już całości)
        if max_new_tokens_calc <= 0 or input_len >= self.max_length:
            print(f'Ostrzeżenie: Długość wejścia ({input_len}) >= max_length ({self.max_length}). Zwracam pustą listę.')
            # Zwracamy listę pustych stringów, aby uniknąć błędów dalej, ale logujemy problem
            return ['' for _ in range(return_num)]
